...
import 'package:republic_on_volga/libs/lib/widgets/preview_image.dart';
...
class SimpleUrlPreview extends StatefulWidget {
  ...
  SimpleUrlPreview({
    ...
    this.onTap,
  })  : assert(previewHeight >= 130.0, 'The preview height should be greater than or equal to 130'),
        assert(
            titleLines <= 2 && titleLines > 0, 'The title lines should be less than or equal to 2 and not equal to 0'),
...
class _SimpleUrlPreviewState extends State<SimpleUrlPreview> {
  ...
  void _extractOGData(Document document, Map data, String parameter) {
    var titleMetaTag = document
        ...
        ?.firstWhere((meta) => meta.attributes['property'] == parameter, orElse: () => null);
